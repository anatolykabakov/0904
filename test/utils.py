...
def plot_map(current_map):
    plt.imshow(current_map, vmin=0, vmax=1, origin='lower', interpolation='none', alpha=1)
    
def plot_path(path_px):
    path_arr = np.array(path_px)
    plt.plot(path_arr[:, 0], path_arr[:, 1], 'y')

    # plot start point
    start_x, start_y = path_arr[0]
    plt.plot(start_x, start_y, 'ro')

    # plot goal point
    goal_x, goal_y = path_arr[-1]
    plt.plot(goal_x, goal_y, 'go')
...
def trajectory2line(trajectory):
    line = ''
    for i in trajectory:
        line += str(i[0])+str(',')+str(i[1])+str(' ')
        return line[:-1]
